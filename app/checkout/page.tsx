  ...
  return (
    <div className="min-h-screen pt-24 pb-16 px-4 sm:px-6 lg:px-8">
      <div className="max-w-4xl mx-auto">
        <h1 className="font-display font-bold text-4xl text-white mb-2">Review Order</h1>
        <p className="text-white/50 mb-10">Double-check your items before heading to Stripe.</p>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-5 gap-8">
          ...
          <div className="lg:col-span-3 space-y-3">
            {cart?.items.map((item) => (
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  <p className="font-semibold text-white line-clamp-2 text-sm">{item.product.name}</p>
                  {item.product.set && (
                    <p className="text-xs text-white/40 mt-0.5">{item.product.set}</p>
                  )}
                  <p className="text-xs text-white/50 mt-1">Qty: {item.quantity}</p>
                </div>
                <p className="font-bold text-gold text-sm flex-shrink-0">
            ...
            <div className="pt-2">
              <Link href="/cart" className="text-sm text-gold hover:text-gold-light transition-colors">
                ← Edit Cart
              </Link>
            </div>
          </div>
          ...
          <div className="lg:col-span-2">
            <div className="rounded-2xl border border-surface-border bg-surface p-6 sticky top-24 space-y-4">
              <h2 className="font-display font-bold text-xl text-white">Order Total</h2>
              ...
              <div className="space-y-3">
                ...
                <div className="flex justify-between text-sm">
                  <span className="text-white/60">Est. Tax (8%)</span>
